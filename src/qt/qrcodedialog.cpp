...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("WKC")
    updateDisplayUnit();
}
...
{
    QString ret = QString("wankcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non WKC unit input in lnReqAmount, we generate the URI with WKC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::WKC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
